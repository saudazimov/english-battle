...
function validateRow(row, index) {
  ...
  const missing = Object.entries(normalized).find(([, value]) => !value);
  if (missing) {
  ...
  async function insertStudent(client, profile) {
    for (let attempt = 0; attempt < 12; attempt += 1) {
      ...
      const result = await client.query(
        `INSERT INTO users
          (first_name, last_name, phone, password, role, username, school, class_name, country)
         VALUES ($1,$2,NULL,$3,'student',$4,$5,$6,'UZ')
         ON CONFLICT DO NOTHING
         RETURNING id`,
        [profile.firstName, profile.lastName, passwordHash, login, profile.school, profile.className]
      );
      if (result.rows[0]) {
  ...
  async function resetPassword(studentId) {
    ...
    const result = await pool.query(
      `UPDATE users SET password=$1
       ...
       RETURNING id, first_name, last_name, username, school, class_name`,
